...
Bullet enemyBullet[2*MAX_BULLET];
Enemy enemy[MAX_ENEMY];
#define MAX_SPAWN_RATE 20000
...
void updateEnemyBullet(uint8_t dt) {
	for(int j=0; j<MAX_ENEMY;j++) {
		...
		for(int i=0; i<MAX_BULLET;i++) {
			...
			enemyBullet[i].updateCoordinate(enemy[j].coordinateX + 5, enemy[j].coordinateY + 16);
			enemyBullet[i].updateDisplayStatus(SHOULD_SHOW);

			enemyBullet[i].updateCoordinate(enemy[j].coordinateX + 15, enemy[j].coordinateY + 16);
				enemyBullet[i].updateDisplayStatus(SHOULD_SHOW);
